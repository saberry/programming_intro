...
Now we are starting to get into some more interest ideas. A list is another way to collect objects. However, we are now freed from the constraint that everything needs to be of the same type.
...
Now is the point in which R gets wacky. In the most simple way, a data frame is a special type of list; really, a data frame is a list with some strict rules. Chief among those rules is that every element of the data frame needs to contain the same number of items in each column.

```{r}
df_example <- data.frame(name = c("seth", "sharif",
                                  "joe", "hexuan"), 
                         age = c(30, 30, 23, 25), 
                         height = c(76, 76, 64, 64), 
